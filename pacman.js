...
var focus = 0;
// Adding this to easily change the amount by which the image should move each time.
var moveAmount = 20;
...
function Run() {
    let img = document.getElementById('PacMan'); //See index.html
    let imgWidth = img.width; //See index.html
...
// Inside of the Run() function you will also have to add an extra argument "pageWidth", which is declared on 
//line 4 when you call the checkPageBounds() function below.
...
function checkPageBounds(direction, imgWidth, pos, pageWidth) {
  ...
  //
  //For the right edge.
  if (direction === 0 && pos >= pageWidth - imgWidth - moveAmount) {
      direction = 1;
  }
  //For the left edge, which is always 0.
...
//Only repeat if the user has clicked and Run() has executed once.
setInterval(Run, 200);
